...
particle pTrack::findnextpoint(particle p) {
	...
	p2.p= p.p + k1 * d*m*(1.0 / 4.0);
	calcVel(p2);
	k2 = p2.v;

	p3.p = p.p + ( k1*(3.0 / 32.0) + k2 * (9.0 / 32.0) ) * (3.0 / 8.0)*d*m;
	calcVel(p3);
	k3 = p3.v;

	p4.p = p.p + ( k1*(1932.0 / 2197.0) + k2*(-(7200.0 / 2197.0)) + k3* (7296.0 / 2197.0) ) * (12.0 / 13.0)*d*m;
	calcVel(p4);
	k4 = p4.v;

	p5.p = p.p + ( k1*(439.0 / 216.0) + k2*(-8.0) + k3*(3680.0 / 513.0) + k4*(-(845.0 / 4104.0)) ) * (1.0)*d*m;
	calcVel(p5);
	k5 = p5.v;

	p6.p = p.p + ( k1*(-(8.0 / 27.0)) + k2*2.0 + k3*(-(3544.0 / 2565.0)) + k4* (1859.0 / 4104.0) + k5*(-(11.0 / 40.0)) ) * (1.0 / 2.0)*d*m;
	calcVel(p6);
	k6 = p6.v;

	yn.p = p.p + ( k1*(25.0 / 216.0) + k3*(1408.0 / 2565.0) + k4* (2197.0 / 4101.0) + k5*(-(1.0 / 5.0)) ) * d*m;
	zn.p = p.p + ( k1*(16.0 / 135.0) + k3*(6656.0 / 12825.0) + k4*(28561.0 / 56430.0) + k5*(-(9.0 / 50.0)) + k6* (2.0 / 55.0) ) * d*m;
